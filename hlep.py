...
# --- FUNÇÃO DE ALINHAMENTO --- (REMOVIDA)
...
def perform_analysis(captured_image):
    ...
    if not results.pose_landmarks:
        return None, None, None  # Modificado: removido o 0 do correction_angle
    ...
    if get_landmark_coords(landmarks_all, mp_pose.PoseLandmark.RIGHT_HIP.value) is None or \
        ...
        print("Pontos críticos (Quadris) não visíveis para a análise. Parando a análise.")  # Mensagem ajustada
        angles = {
        ...
        # Modificado: retorna a imagem original e os landmarks detectados (mesmo com erro)
        return captured_image, angles, results.pose_landmarks

        # Linhas de rotação removidas
    ...
    # Usa os landmarks diretamente dos resultados originais
    ...
    # Modificado: Retorna a imagem original, os ângulos e os landmarks originais
    return captured_image, calculated_angles, results.pose_landmarks


# --- FUNÇÃO PARA SALVAR TXT  ---

def save_angles_to_txt(angles_dict):  # Modificado: removido correction_angle
    """Salva os ângulos calculados em um arquivo TXT."""  # Descrição atualizada
...
def main_camera_capture():
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose_preview:
        ...
        while True:
            ret, frame = cap.read()
            if not ret: break
            ###frame = cv2.flip(frame, 1) # Flip horizontal (espelho) para o preview
    ...
    if captured_frame is not None:
        # Modificado: chamada de função atualizada (sem correction_angle)
        final_image, angles, final_landmarks_result = perform_analysis(captured_frame)

        if final_image is not None and angles is not None:
            if "ERRO" in angles:
                save_angles_to_txt(angles)  # Modificado: chamada atualizada
            ...
            # final_image=cv2.flip(final_image,1) # <-- LINHA REMOVIDA PARA CORRIGIR A IMAGEM
